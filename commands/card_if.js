...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        let Li = a * b ; // 公式好像怪怪的，應該要如何更改呢?
        let Si = c * d ;
        if (Li < Si) {
            // 如何判斷 大或小
            await interaction.reply(`史考特勝利  (利奧拉:${Li}史考特:${Si})`); // 如果符合，應該回覆甚麼訊息
        } else if (Li > Si) {
            // 應該要如何判斷剩下的
            await interaction.reply(`利奧拉勝利  (利奧拉:${Li}史考特:${Si})`); // 如果符合，應該回覆甚麼訊息
        } else {
            // 最後一個應該是甚麼情況
            await interaction.reply(`平手  (利奧拉:${Li}史考特:${Si})`); // 如果符合，應該回覆甚麼訊息
        }
    },
};
